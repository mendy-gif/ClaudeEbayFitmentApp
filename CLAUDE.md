...
2. **Two compatibility stores that can diverge:** Inventory API (by **SKU**, what we *write* via
   `createOrReplaceProductCompatibility`) vs Trading API (by **ItemID**, what *displays* and survives
   relist, read via `GetItem` with the `X-EBAY-API-IAF-TOKEN` header). The "already expanded" guard
   reads Trading; writes go to Inventory.
...
6. **Over-including fitment is safe:** eBay drops catalog rows it can't validate (partial-accept warning
   25023), so padding years / unioning both sources won't create bad listings.
7. **The ledger prevents double-work** but only records *pushes*; skips are re-evaluated each run (that's
   why the same "already N vehicles / no donor" lines recur — harmless).
...
## Sibling subproject: `bmw-etk/` (BMW parts catalogue)

A separate effort on branch **`claude/bmw-etk-database-sqohoo`** turns a BMW ETK
dealer parts-catalogue disc into a **part number → vehicle/chassis** table, intended
as a **third fitment source** alongside the chassis rules and part-number history.
Later goals: a VIN decoder, and a VIN + part number fitment checker.

It has its own project memory at **`bmw-etk/CLAUDE.md`** — read that before touching
anything under `bmw-etk/`. Two things to know from here:

- The two projects live on **different branches**, so `bmw-etk/` files are invisible
  while the eBay branch is checked out (and vice versa). That is expected.
- The raw catalogue dump is multi-gigabyte and **must never be committed**; the
  gitignore blocks `bmw-etk/dump/` and every database-ish file extension.
